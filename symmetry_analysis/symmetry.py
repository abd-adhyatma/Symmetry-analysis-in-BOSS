...
def get_symmetric_positions(
    pos: list, slab: Atoms, x_bounds: list = [0, 1], y_bounds: list = [0, 1], 
    ...
    Parameters:
    pos (list): containing scaled coordinates x, y, z of molecule center of mass position on slab.
    ...
    """
    # Getting symmetry information from slab
    cell = get_spglib_cell(slab)
    symmetry = filter_z(get_symmetry(cell, 1e-5))
    
    # Convert initial pos from BOSS to symmetry coordinates
    pos = boss_to_symm(pos, slab)

    # Initiating empty list for unique positions with transformations
    unique_positions = []
    ...
    # Open the file for writing symmetry operations and checks
    with open(output_filename, 'w') as file:
        file.write("Symmetry Operations Check Log\n")
        file.write("=" * 80 + "\n")
        file.write("Index | Pass/Fail | Failed Check | Determinant | W_surf | t_surf\n")
        ...
        for i, (rotation, translation) in enumerate(zip(rotations, translations)):
            ...
            # Get text formats
            rotation_text = rotation.tolist()
            translation_text = translation.tolist()
                
            # Apply the symmetry operation for positions: rotate and translate
            new_pos = np.dot(rotation, pos) + translation
            
            # Ensure periodic boundary conditions (positions should be between 0 and 1)
            new_pos = new_pos % 1.0
            
            # Convert the new position back from symmetry coordinates to BOSS format
            new_pos_boss = symm_to_boss(new_pos, slab)
            
            if x_bounds[0] <= new_pos_boss[0] <= x_bounds[1] and y_bounds[0] <= new_pos_boss[1] <= y_bounds[1]:
                if not any(np.allclose(new_pos_boss, existing_pos, atol=tolerance) for existing_pos, _ in unique_positions):
                    unique_positions.append(
                        (np.array(new_pos_boss), 
                         (rotation, translation)))
                    file.write(f"{i}    | Pass     | None | {determinant:.3f} | {rotation_text} | {translation_text}\n")
                else:
                    file.write(f"{i}    | Fail     | Non-unique position | {determinant:.3f} | {rotation_text} | {translation_text}\n")
    
    return unique_positions

def get_symmetric_positions_and_angles(
    pos: list, angles: list, slab: Atoms, mol: Atoms, mol_symmetries=None, idx=None, 
    ...
    slab (ase.Atoms object): slab system isolated from the adsorbed molecule.
    mol (ase.Atoms object): molecule isolated from the slab.
    mol_symmetries (if not None, list): list of molecule symmetries in rotation matrix form
